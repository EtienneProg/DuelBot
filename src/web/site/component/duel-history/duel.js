...
<div class="relative ${data.estTeam ? " h-2/6" : "h-1/6"} grid min-[546px]:grid-cols-[1fr_auto_1fr] max-[545px]:grid-rows-[1fr_auto_1fr] bg-[#2a1747] items-center w-full rounded-2xl px-4 py-2 ${data.duelEnCours ? "" :"border border-green-400/70"}">
    ...
    <div class="min-[546px]:justify-self-start h-full flex flex-col">
    ${ data.team1.map(player => duelPlayerComponentT1(player, data.duelEnCours))}
    </div>
    ...
    <div class="min-[546px]:justify-self-end h-full">
        ${ data.team2.map(player => duelPlayerComponentT2(player, data.duelEnCours))}
    </div>
</div>
`
